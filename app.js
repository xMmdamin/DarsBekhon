...
/* ===================== STATISTICS PAGE (YPT-inspired) ===================== */
(function(){
  let statMode = "period";
  const STAT_PERIOD_DAYS = 28;
  const STAT_SUBJECT_COLORS = [
    "#53e6a6","#6ee7ff","#9b8cff","#ffd166","#ff6b81",
    "#e879f9","#4ade80","#38bdf8","#a78bfa","#fb923c",
    "#22d3ee","#f472b6","#facc15","#34d399","#818cf8"
  ];

  function statGetRange(){
    const now = new Date(); now.setHours(0,0,0,0);
    const d = new Date(now);
    switch(statMode){
      case "day":
        return { from: d, to: new Date(d.getTime()+86400000), days: 1 };
      case "week": {
        const dow = (d.getDay() + 1) % 7; // Sat=0
        d.setDate(d.getDate() - dow);
        const end = new Date(d); end.setDate(end.getDate()+7);
        return { from: d, to: end, days: 7 };
      }
      case "month": {
        const j = gregorianToJalali(d.getFullYear(), d.getMonth()+1, d.getDate());
        const firstG = jalaliToGregorian(j.jy, j.jm, 1);
        const daysInMonth = j.jm<=6?31:(j.jm<=11?30:(j.jy%4===3?30:29));
        const end = new Date(firstG); end.setDate(end.getDate()+daysInMonth);
        return { from: firstG, to: end, days: daysInMonth };
      }
      case "trend": {
        const from = new Date(d); from.setDate(from.getDate()-180);
        return { from, to: new Date(d.getTime()+86400000), days: 180 };
      }
      case "period":
      default: {
        const from = new Date(d); from.setDate(from.getDate()-(STAT_PERIOD_DAYS-1));
        return { from, to: new Date(d.getTime()+86400000), days: STAT_PERIOD_DAYS };
      }
    }
  }

  function statEpisodesInRange(){
    const { from, to } = statGetRange();
  ...
  function statFormatMinutes(m){
    if(!m) return "۰ دقیقه";
    if(m<60) return fmt(m)+" دقیقه";
    const h=Math.floor(m/60), mm=m%60;
    return mm===0 ? fmt(h)+" ساعت" : fmt(h)+" ساعت و "+fmt(mm)+" دقیقه";
  }
  function statShortMinutes(m){
    if(!m) return "۰";
    if(m<60) return fmt(m)+"د";
    return (m/60).toFixed(1).replace(/\.0$/,"")+"س";
  }

  function subjectTotals(eps){
    const totals = {};
    eps.forEach(e=>{
      const k = e.subject || "unknown";
      totals[k] = (totals[k]||0) + (+e.minutes||0);
    });
    return Object.entries(totals).sort((a,b)=>b[1]-a[1]);
  }

  function buildDailyMatrix(eps){
    const map = {};
    eps.forEach(e=>{
      const k = String(e.date||"").slice(0,10);
      if(!map[k]) map[k] = {};
      const sub = e.subject || "unknown";
      map[k][sub] = (map[k][sub]||0) + (+e.minutes||0);
    });
    return map;
  }

  function renderSubjectPie(eps){
    const svg = document.getElementById("statSubjectPie");
    const legend = document.getElementById("statSubjectLegend");
    if(!svg||!legend) return;
    const totals = subjectTotals(eps);
    if(!totals.length){
      svg.innerHTML="";
      legend.innerHTML='<div class="empty">داده‌ای در این بازه نیست.</div>';
      return;
    }
    ...
    totals.forEach(([k,v],i)=>{
      const a = v/sum * Math.PI*2;
      const end = angle + a;
      const x1=cx+Math.cos(angle)*r, y1=cy+Math.sin(angle)*r;
      const x2=cx+Math.cos(end)*r,   y2=cy+Math.sin(end)*r;
      const ix1=cx+Math.cos(angle)*ir, iy1=cy+Math.sin(angle)*ir;
      const ix2=cx+Math.cos(end)*ir,   iy2=cy+Math.sin(end)*ir;
      const large = a>Math.PI?1:0;
      const color = STAT_SUBJECT_COLORS[i%STAT_SUBJECT_COLORS.length];
      if(totals.length===1){
        paths += `<circle cx="${cx}" cy="${cy}" r="${(r+ir)/2}" fill="none" stroke="${color}" stroke-width="${r-ir}" opacity="0.9"/>`;
      }else{
        paths += `<path d="M ${x1} ${y1} A ${r} ${r} 0 ${large} 1 ${x2} ${y2} L ${ix2} ${iy2} A ${ir} ${ir} 0 ${large} 0 ${ix1} ${iy1} Z" fill="${color}" opacity="0.88"/>`;
      }
      angle = end;
    });
    svg.innerHTML = paths;
    legend.innerHTML = totals.map(([k,v],i)=>{
      const pct = Math.round(v/sum*100);
      const color = STAT_SUBJECT_COLORS[i%STAT_SUBJECT_COLORS.length];
      const name = SUBJECTS[k]?.name || subjectDisplayName(k,cur) || k;
      const icon = SUBJECTS[k]?.icon || "📘";
      return `<div class="stat-legend-row">
        <span class="stat-legend-dot" style="background:${color}"></span>
        <span class="stat-legend-name">${icon} ${esc(name)}</span>
        <span class="stat-legend-value">${statShortMinutes(v)} • ${pct}٪</span>
      </div>`;
    }).join("");
  }

  function renderDailyBars(eps, range){
    const el = document.getElementById("statDailyBars");
    if(!el) return;
    const matrix = buildDailyMatrix(eps);
    const totals = subjectTotals(eps);
    if(!totals.length){
      el.innerHTML='<div class="empty">داده‌ای برای نمایش نیست.</div>';
      return;
    }
    const orderedSubjects = totals.map(x=>x[0]);
    const dayKeys = [];
    for(let i=0;i<range.days;i++){
      const d = new Date(range.from); d.setDate(d.getDate()+i);
      dayKeys.push(d.toISOString().slice(0,10));
    }
    const max = Math.max(1, ...dayKeys.map(k=>{
      const day = matrix[k]||{};
      return Object.values(day).reduce((a,b)=>a+b,0);
    }));
    const step = range.days>14 ? Math.ceil(range.days/8) : 1;
    let bars = "";
    dayKeys.forEach((k,i)=>{
      const day = matrix[k]||{};
      const total = Object.values(day).reduce((a,b)=>a+b,0);
      let stack = "";
      orderedSubjects.forEach((subj,si)=>{
        const val = day[subj]||0;
        if(!val) return;
        const h = val/max*100;
        const color = STAT_SUBJECT_COLORS[si%STAT_SUBJECT_COLORS.length];
        stack += `<div class="stat-bar-seg" style="height:${h}%;background:${color}" title="${esc(SUBJECTS[subj]?.name||subj)} • ${statShortMinutes(val)}"></div>`;
      });
      const showLabel = (i%step===0 || i===range.days-1);
      const d = new Date(k);
      const lab = showLabel ? d.toLocaleDateString("fa-IR",{month:"numeric",day:"numeric"}) : "";
      bars += `<div class="stat-bar-col">
        <div class="stat-bar-track" title="${total?statFormatMinutes(total):"بدون مطالعه"}">${stack}</div>
        <span class="stat-bar-label">${lab}</span>
      </div>`;
    });
    el.innerHTML = `<div class="stat-bar-chart">${bars}</div>`;
  }

  function renderCumulative(eps, range){
    const svg = document.getElementById("statCumulativeChart");
    if(!svg) return;
    const totals = subjectTotals(eps);
    if(!totals.length){
      svg.innerHTML='<text x="260" y="115" text-anchor="middle" fill="var(--muted)">داده‌ای نیست</text>';
      return;
    }
    const orderedSubjects = totals.map(x=>x[0]);
    const dayKeys = [];
    ...
    const matrix = buildDailyMatrix(eps);
    const W=520,H=220,P={l:36,r:12,t:20,b:26};
    const innerW = W-P.l-P.r, innerH = H-P.t-P.b;

    // cumulative per subject series
    const perSubjSeries = orderedSubjects.map(subj=>{
      let acc = 0;
    ...
    const maxVal = Math.max(1, ...perSubjSeries.flat());

    let svgC = "";
    for(let i=0;i<=3;i++){
      const y = P.t + i*innerH/3;
      svgC += `<line x1="${P.l}" x2="${W-P.r}" y1="${y}" y2="${y}" stroke="rgba(125,145,190,.14)" stroke-width="1"/>`;
    }
    const baseAcc = new Array(range.days).fill(0);
    orderedSubjects.forEach((subj,si)=>{
      const color = STAT_SUBJECT_COLORS[si%STAT_SUBJECT_COLORS.length];
      const series = perSubjSeries[si];
      const topPts = [], botPts = [];
      dayKeys.forEach((k,i)=>{
        const x = P.l + i*innerW/Math.max(1,range.days-1);
        const yTop = H-P.b - (baseAcc[i]+series[i])/maxVal*innerH;
        const yBot = H-P.b - baseAcc[i]/maxVal*innerH;
        topPts.push({x,y:yTop});
        botPts.push({x,y:yBot});
      });
      const topPath = topPts.map((q,i)=>(i?"L":"M")+q.x.toFixed(1)+" "+q.y.toFixed(1)).join(" ");
      const botPath = botPts.slice().reverse().map(q=>"L"+q.x.toFixed(1)+" "+q.y.toFixed(1)).join(" ");
      svgC += `<path d="${topPath} ${botPath} Z" fill="${color}" opacity="0.78"/>`;
      dayKeys.forEach((_,i)=>{ baseAcc[i] += series[i]; });
    });
    const stepX = range.days>14 ? Math.ceil(range.days/6) : Math.max(1, Math.ceil(range.days/5));
    dayKeys.forEach((k,i)=>{
      if(i%stepX!==0 && i!==range.days-1) return;
      const x = P.l + i*innerW/Math.max(1,range.days-1);
      const d = new Date(k);
      svgC += `<text x="${x}" y="${H-8}" text-anchor="middle" fill="var(--muted)" font-size="9">${d.toLocaleDateString("fa-IR",{month:"numeric",day:"numeric"})}</text>`;
    });
    svg.innerHTML = svgC;
  }

  function renderWeekdayBars(eps){
    const el = document.getElementById("statWeekdayBars");
    if(!el) return;
    ...
    eps.forEach(e=>{
      ...
      sums[dow] += +e.minutes||0;
    });
    const range = statGetRange();
    ...
    el.innerHTML = names.map((n,i)=>{
      const h = avgs[i] ? Math.max(4, Math.round(avgs[i]/max*100)) : 0;
      return `<div class="stat-weekday-col">
        <span class="stat-weekday-val">${avgs[i]?statShortMinutes(avgs[i]):"۰"}</span>
        <div class="stat-weekday-track"><div class="stat-weekday-fill" style="height:${h}%"></div></div>
        <span class="stat-weekday-label">${n.slice(0,3)}</span>
      </div>`;
    }).join("");
  }

  function renderStartEnd(eps){
    const el = document.getElementById("statStartEndScatter");
    if(!el) return;
    if(!eps.length){ el.innerHTML='<div class="empty">داده‌ای برای نمایش نیست.</div>'; return; }
    const W=520,H=240,P={l:44,r:12,t:16,b:34};
    const innerW = W-P.l-P.r, innerH = H-P.t-P.b;
    const range = statGetRange();
    const startBase = new Date(range.from.getFullYear(),range.from.getMonth(),range.from.getDate(),12,0,0);
    const points = [];
    eps.forEach(e=>{
      const k = String(e.date||"").slice(0,10);
      const d = new Date(k+"T12:00:00");
      const dayIdx = Math.round((d - startBase)/86400000);
      const tm = String(e.time||"").match(/(\d{1,2}):(\d{2})/);
      if(!tm) return;
      const startMin = (+tm[1])*60 + (+tm[2]);
      points.push({dayIdx, startMin, endMin: startMin + (+e.minutes||0)});
    });
    if(!points.length){ el.innerHTML='<div class="empty">ساعت‌های مطالعه ثبت نشده.</div>'; return; }
    const yFor = m => P.t + (Math.max(0,Math.min(1440,m))/1440)*innerH;
    const xFor = idx => P.l + (idx/Math.max(1,range.days-1))*innerW;
    let svg = "";
    [0,6,12,18,24].forEach(h=>{
      const y = yFor(h*60);
      svg += `<line x1="${P.l}" x2="${W-P.r}" y1="${y}" y2="${y}" stroke="rgba(125,145,190,.15)" stroke-dasharray="2 3"/>`;
      svg += `<text x="${P.l-6}" y="${y+3}" text-anchor="end" fill="var(--muted)" font-size="9">${String(h).padStart(2,"0")}:00</text>`;
    });
    const step = range.days>14?Math.ceil(range.days/6):Math.max(1,Math.ceil(range.days/5));
    for(let i=0;i<range.days;i+=step){
      const d = new Date(range.from); d.setDate(d.getDate()+i);
      const x = xFor(i);
      svg += `<text x="${x}" y="${H-8}" text-anchor="middle" fill="var(--muted)" font-size="9">${d.toLocaleDateString("fa-IR",{month:"numeric",day:"numeric"})}</text>`;
    }
    points.forEach(p=>{
      const x = xFor(p.dayIdx);
      const y1 = yFor(p.startMin);
      const y2 = yFor(p.endMin);
      svg += `<line x1="${x}" y1="${y1}" x2="${x}" y2="${y2}" stroke="var(--accent)" stroke-width="3" opacity="0.55"/>`;
      svg += `<circle cx="${x}" cy="${y1}" r="3.5" fill="var(--accent)"/>`;
      svg += `<circle cx="${x}" cy="${y2}" r="3.5" fill="var(--accent2)"/>`;
    });
    el.innerHTML = `<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="width:100%;height:220px">${svg}</svg>
      <div class="stat-startend-legend">
        <span><span class="stat-legend-dot" style="background:var(--accent)"></span>شروع</span>
        <span><span class="stat-legend-dot" style="background:var(--accent2)"></span>پایان</span>
      </div>`;
  }

  function renderTimeline(eps){
    const el = document.getElementById("statTimeline");
    if(!el) return;
    if(!eps.length){ el.innerHTML='<div class="empty">جلسه‌ای در این بازه ثبت نشده.</div>'; return; }
    ...
    el.innerHTML = sorted.map(e=>{
      const v = SUBJECTS[e.subject] || {icon:"📘", name:e.subject};
      const name = e.subjectLabel || subjectDisplayName(e.subject,cur) || v.name;
      return `<div class="stat-timeline-row">
        <div class="stat-timeline-time">${esc(e.time||"—")}</div>
        <div class="stat-timeline-dot"></div>
        <div class="stat-timeline-body">
          <b>${v.icon} ${esc(name)}</b>
          <small>${esc(e.topic||"بدون موضوع")} • ${statShortMinutes(+e.minutes||0)}</small>
        </div>
        <div class="stat-timeline-date">${esc(e.date||"")}</div>
      </div>`;
    }).join("");
  }

  window.renderStatisticsPage = function(){
    const eps = statEpisodesInRange();
    const range = statGetRange();
    const totals = subjectTotals(eps);
    const totalMin = totals.reduce((a,b)=>a+b[1],0);
    const uniqueDays = new Set(eps.map(e=>String(e.date||"").slice(0,10)));
    const activeDays = uniqueDays.size;
    const dailyAvg = range.days ? Math.round(totalMin/range.days) : 0;
    const setTxt = (id,v)=>{ const e=document.getElementById(id); if(e) e.textContent=v; };
    setTxt("statTotalTime", statFormatMinutes(totalMin));
    setTxt("statDailyAvg", statFormatMinutes(dailyAvg));
    setTxt("statSessionCount", fmt(eps.length));
    setTxt("statActiveDays", `${fmt(activeDays)} / ${fmt(range.days)}`);
    const info = document.getElementById("statRangeInfo");
    if(info){
      const fD = d => new Intl.DateTimeFormat("fa-IR",{month:"short",day:"numeric"}).format(d);
      info.textContent = `${fD(range.from)} تا ${fD(new Date(range.to.getTime()-86400000))} • ${fmt(range.days)} روز`;
    }
    renderSubjectPie(eps);
    renderDailyBars(eps, range);
    renderCumulative(eps, range);
    renderWeekdayBars(eps);
    renderStartEnd(eps);
    renderTimeline(eps);
  };

  window.setStatMode = function(mode){
    statMode = mode || "period";
    document.querySelectorAll(".stat-mode-btn").forEach(b=>b.classList.toggle("active", b.dataset.mode===mode));
    renderStatisticsPage();
  };

  const _origSwitch = window.switchToPage;
  window.switchToPage = function(name){
    _origSwitch(name);
    if(name === "statistics") renderStatisticsPage();
  };

  const _origRender = window.render;
  window.render = function(){
    _origRender();
    const p = document.getElementById("statistics");
    if(p && p.classList.contains("active")) renderStatisticsPage();
  };

  if(document.getElementById("statistics")?.classList.contains("active")) renderStatisticsPage();
  console.log("✅ Statistics page loaded (YPT-inspired)");
})();