...
english:(function(){
    ...
    return {name:"انگلیسی",icon:"🔤",cols,sections:[
      {id:"g10",label:"پایه دهم (ویژن ۱)",rows:rowsByLesson("g10",4,topicsNoColl,
        ["درس ۱","درس ۲","درس ۳","درس ۴"])},
      {id:"g11",label:"پایه یازدهم (ویژن ۲)",rows:rowsByLesson("g11",3,topicsNoColl,
        ["درس ۱","درس ۲","درس ۳"])},
      {id:"g12",label:"پایه دوازدهم (ویژن ۳)",rows:rowsByLesson("g12",3,topicsFull,
        ["درس ۱","درس ۲","درس ۳"])}
    ]};
  })(),
...
const PAGE_NAV_ITEMS=[
  ["dashboard","🏠 خانه"],["statistics","📈 آمار"],["episodes","📖 پارت‌های مطالعه"],["checklist","📋 چک‌لیست"],
  ["classes","📅 برنامه روزانه"],["routines","🔁 روتین"],["quests","📋 کارهای امروز"],["mistakes","❌ دفترچه اشتباهات"],
  ["mockExams","🧪 آزمون‌ها"],["tasks","✅ عادت‌ها"],["skillHub","📊 نقشه تسلط"],["leitner","🧰 جعبه ابزار"],["settings","⚙️ تنظیمات"]
];
function renderDesktopNav(){
  const wrap=document.getElementById("navQuick"); if(!wrap)return;
  wrap.innerHTML=PAGE_NAV_ITEMS.map(([id,label])=>`<button type="button" class="nav-tab" data-page="${id}" onclick="switchToPage('${id}')">${label}</button>`).join("");
  updateDesktopNav();
}
function updateDesktopNav(){
  const active=document.querySelector(".page.active")?.id||"dashboard";
  document.querySelectorAll(".nav-tab").forEach(b=>b.classList.toggle("active",b.dataset.page===active));
  const sel=document.getElementById("pageSelect"); if(sel)sel.value=active;
}
window.switchToPage=function(name){
  const page=document.getElementById(name); if(!page)return;
  ...
  }catch(err){console.error("Page render error:",name,err);}
...
function taskDoneCount(taskId, days=7){
  let n=0; const now=new Date();
  for(let i=0;i<days;i++){
    const d=new Date(now); d.setDate(d.getDate()-i);
    const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    if(state.tasksDone?.[key]?.includes(taskId)) n++;
  }
  return n;
}
function taskStreak(taskId){
  ...
  for(let i=0;i<365;i++){
    const d=new Date(now); d.setDate(d.getDate()-i);
    const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    if(!state.tasksDone?.[key]?.includes(taskId)) break;
    n++;
  }
  return n;
}
function taskWeekRate(taskId){return Math.round(taskDoneCount(taskId,7)/7*100)}
...
function renderTasks(){
  ...
  const done=all.filter(t=>isTaskDoneToday(t.id)).length;
  const pct=total?Math.round(done/total*100):0;

  if(summary) summary.innerHTML=`
    <div class="ts-item ts-progress">
      <span>پیشرفت امروز</span>
      <b>${fmt(done)} / ${fmt(total)}</b>
      <i><em style="width:${pct}%"></em></i>
    </div>
    <div class="ts-item"><span>درصد انجام</span><b>${fmt(pct)}٪</b></div>
    <div class="ts-item"><span>باقی‌مانده</span><b>${fmt(Math.max(0,total-done))}</b></div>
  `;

  grid.innerHTML=Object.entries(TASK_CATS).map(([catId,cat])=>{
    const catTasks=all.filter(t=>t.cat===catId);
    const catDone=catTasks.filter(t=>isTaskDoneToday(t.id)).length;
    const catPct=catTasks.length?Math.round(catDone/catTasks.length*100):0;

    return `<article class="task-cat">
      <div class="task-cat-head">
        <div class="task-cat-title">
          <span class="task-cat-icon">${cat.icon}</span>
          <div><h3>${cat.name}</h3><small>${catTasks.length?`${catDone} از ${catTasks.length} عادت انجام شده`:'هنوز عادتی ثبت نشده'}</small></div>
        </div>
        <span class="task-cat-pct">${fmt(catPct)}٪</span>
      </div>
      <div class="task-cat-progress"><i style="width:${catPct}%"></i></div>
      <div class="task-list">
        ${catTasks.length ? catTasks.map(t=>{
          const isDone=isTaskDoneToday(t.id);
          return `<div class="task-item ${isDone?"done":""}" onclick="toggleTask('${t.id}')" role="button" tabindex="0"
              onkeydown="if(event.key==='Enter'||event.key===' ')toggleTask('${t.id}')">
            <div class="task-check" aria-hidden="true"></div>
            <div class="task-info"><span class="task-name"><span class="task-icon">${t.icon||"⭐"}</span>${esc(t.name)}<small class="task-meta">هفته: ${fmt(taskWeekRate(t))}٪ • پیوستگی: ${fmt(taskStreak(t))} روز</small></span></div>
            <div class="task-actions" onclick="event.stopPropagation()">
              <button class="task-icon-btn" onclick="openTaskEditModal('${t.id}')" title="ویرایش" aria-label="ویرایش">✏️</button>
              <button class="task-icon-btn task-delete" onclick="deleteTask('${t.id}')" title="حذف" aria-label="حذف">🗑</button>
            </div>
          </div>`;
        }).join("") : `<div class="task-empty">برای این بخش هنوز عادتی اضافه نشده.</div>`}
      </div>
      <div class="task-add-row">
        <input class="tn" id="taskName_${catId}" maxlength="60" placeholder="مثلاً مطالعه آزاد..." onkeydown="if(event.key==='Enter')addCustomTask('${catId}')">
        <button class="btn primary" onclick="addCustomTask('${catId}')">＋ افزودن</button>
      </div>
    </article>`;
  }).join("");
}
...
const SKILL_SECONDTERM_BOOKS_10={
  experimental:["فارسی ۱","دین و زندگی ۱ (مشترک)","عربی ۱ (مشترک)","انگلیسی ۱","جغرافیای ایران","آمادگی دفاعی","تفکر و سواد رسانه‌ای","زیست‌شناسی ۱","شیمی ۱","فیزیک ۱ تجربی","ریاضی ۱"],
  mathematics:["فارسی ۱","دین و زندگی ۱ (مشترک)","عربی ۱ (مشترک)","انگلیسی ۱","جغرافیای ایران","آمادگی دفاعی","تفکر و سواد رسانه‌ای","هندسه ۱","شیمی ۱","فیزیک ۱","ریاضی ۱"],
  humanities:["فارسی ۱","دین و زندگی ۱ (انسانی)","آمادگی دفاعی","تفکر و سواد رسانه‌ای","عربی ۱ (تخصصی)","جغرافیای ایران","تاریخ ۱","ریاضی و آمار ۱","علوم و فنون ادبی ۱","منطق","جامعه‌شناسی ۱","انگلیسی ۱"]
};
...
function skillSecondTermSubjects(curriculum,grade){const g=String(grade);if(g==='10')return SKILL_SECONDTERM_BOOKS_10[curriculum]||[];if(g==='11')return SKILL_SECONDTERM_BOOKS_11[curriculum]||[];return []}
...
(function(){
  ...
  window.addEpisode = function(){
    ...
    const e = {
      id: crypto.randomUUID(), date: today(), ts: Date.now(),
...
/* ===================== STATISTICS (روز/هفته/ماه/مجموع) ===================== */
(function(){
  const JALALI_MONTHS=['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'];
  const FA_WEEKDAYS=['یکشنبه','دوشنبه','سه‌شنبه','چهارشنبه','پنجشنبه','جمعه','شنبه'];
  const STATS_PALETTE=['#4caf50','#8bc34a','#9e9e9e','#03a9f4','#673ab7','#ff9800','#e91e63','#00bcd4','#cddc39','#795548','#3f51b5','#009688','#f44336','#607d8b','#ffc107','#8e44ad','#16a085','#d35400','#2980b9','#c0392b','#27ae60','#f39c12'];
  let statsState={range:'day',anchor:(function(){const d=new Date();d.setHours(12,0,0,0);return d})()};

  function faDigitsToEn(s){const map={'۰':'0','۱':'1','۲':'2','۳':'3','۴':'4','۵':'5','۶':'6','۷':'7','۸':'8','۹':'9'};return String(s??'').replace(/[۰-۹]/g,d=>map[d]);}
  function episodeStartMs(e){
    if(e.ts) return +e.ts;
    const t=faDigitsToEn(e.time||'').match(/(\d{1,2}):(\d{2})/);
    const d=e.date||today();
    if(t){const dt=new Date(d+'T00:00:00');dt.setHours(+t[1],+t[2],0,0);return dt.getTime()}
    return new Date(d+'T12:00:00').getTime();
  }
  function episodeEndMs(e){return episodeStartMs(e)+Math.max(0,(+e.minutes||0))*60000}
  function subjectColor(key){
    const keys=Object.keys(SUBJECTS);let idx=keys.indexOf(key);
    if(idx<0) idx=String(key).split('').reduce((a,c)=>a+c.charCodeAt(0),0);
    return STATS_PALETTE[idx%STATS_PALETTE.length];
  }
  function subjectDisplayNameSafe(k){return (SUBJECTS[k]&&SUBJECTS[k].name)||k||'سایر'}
  function ymd(d){return localKey(d)}
  function addDays(d,n){const x=new Date(d);x.setDate(x.getDate()+n);return x}
  function startOfWeek(d){const x=new Date(d);x.setHours(12,0,0,0);const dow=x.getDay();const diff=(dow+1)%7;return addDays(x,-diff)}
  function episodesBetween(startD,endD){const s=ymd(startD),e=ymd(endD);return state.episodes.filter(ep=>ep.date>=s&&ep.date<=e)}
  function firstEpisodeDate(){if(!state.episodes.length)return today();return state.episodes.reduce((min,e)=>e.date<min?e.date:min,state.episodes[0].date)}
  function subjectBreakdown(episodes){
    const map={};
    episodes.forEach(e=>{const k=e.subject||'other';map[k]=map[k]||{minutes:0,episodes:0,tests:0,correct:0};map[k].minutes+=+e.minutes||0;map[k].episodes++;map[k].tests+=+e.tests||0;map[k].correct+=+e.correct||0;});
    return map;
  }
  function totalMinutes(episodes){return episodes.reduce((a,e)=>a+(+e.minutes||0),0)}
  function fmtDuration(totalMin){
    totalMin=Math.round(totalMin||0);
    const h=Math.floor(totalMin/60),m=totalMin%60;
    if(h<=0)return `${fmt(m)} دقیقه`;
    return `${fmt(h)} ساعت${m?' و '+fmt(m)+' دقیقه':''}`;
  }
  function fmtClock(ms){return new Date(ms).toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'})}
  function faDayLabel(d){const j=gregorianToJalali(d.getFullYear(),d.getMonth()+1,d.getDate());return `${FA_WEEKDAYS[d.getDay()]}، ${fmt(j.jd)} ${JALALI_MONTHS[j.jm-1]} ${fmt(j.jy)}`}
  function faShortDate(d){const j=gregorianToJalali(d.getFullYear(),d.getMonth()+1,d.getDate());return `${fmt(j.jd)} ${JALALI_MONTHS[j.jm-1]}`}
  function jalaliDaysInMonth(jy,jm){return jm<=6?31:(jm<=11?30:(jy%4===3?30:29))}

  /* ---- SVG chart builders ---- */
  function donutSVG(items,opts){
    opts=opts||{};const size=opts.size||170,stroke=opts.stroke||26,r=(size-stroke)/2,c=size/2;
    const circumference=2*Math.PI*r;const total=items.reduce((a,i)=>a+i.value,0)||1;
    let offset=0;
    const segs=items.map(it=>{
      const frac=it.value/total,len=Math.max(0,frac*circumference-1.5);
      const seg=`<circle cx="${c}" cy="${c}" r="${r}" fill="none" stroke="${it.color}" stroke-width="${stroke}" stroke-linecap="round" stroke-dasharray="${len} ${circumference-len}" stroke-dashoffset="${-offset}" transform="rotate(-90 ${c} ${c})"/>`;
      offset+=frac*circumference;return seg;
    }).join('');
    return `<svg viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">${segs}</svg>`;
  }
  function donutBlock(items,total){
    if(!items.length)return `<div class="empty">داده‌ای برای نمایش نیست.</div>`;
    const svg=donutSVG(items,{size:170,stroke:26});
    const legend=items.map(it=>{
      const pct=total?Math.round(it.value/total*100):0;
      return `<div class="stats-legend-row"><i style="background:${it.color}"></i><span>${esc(it.label)}</span><b>${fmtDuration(it.value)}</b><small>${pct}٪</small></div>`;
    }).join('');
    return `<div class="stats-donut-wrap"><div class="stats-donut-svg">${svg}</div><div class="stats-donut-legend">${legend}</div></div>`;
  }
  function barChartSVG(bars,opts){
    opts=opts||{};const w=opts.w||620,h=opts.h||180,pad={l:10,r:10,t:16,b:26};
    const max=Math.max(1,...bars.map(b=>b.value));
    const bw=(w-pad.l-pad.r)/Math.max(1,bars.length);
    let svg='';
    bars.forEach((b,i)=>{
      const bh=(h-pad.t-pad.b)*(b.value/max);
      const x=pad.l+i*bw+bw*0.16,bw2=bw*0.68,y=h-pad.b-Math.max(0,bh);
      svg+=`<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${bw2.toFixed(1)}" height="${Math.max(1,bh).toFixed(1)}" rx="4" fill="${b.color||'var(--accent)'}"/>`;
      if(b.value>0)svg+=`<text x="${(x+bw2/2).toFixed(1)}" y="${(y-5).toFixed(1)}" text-anchor="middle" class="stats-val-label">${fmtDuration(b.value)}</text>`;
      svg+=`<text x="${(x+bw2/2).toFixed(1)}" y="${h-8}" text-anchor="middle" class="stats-bar-label">${esc(b.label)}</text>`;
    });
    return `<svg viewBox="0 0 ${w} ${h}" class="stats-bar-svg">${svg}</svg>`;
  }
  function stackedBarSVG(buckets,subjKeys,opts){
    opts=opts||{};const w=opts.w||620,h=opts.h||200,pad={l:8,r:8,t:10,b:24};
    const totals=buckets.map(b=>subjKeys.reduce((a,k)=>a+(b.values[k]||0),0));
    const max=Math.max(1,...totals);
    const bw=(w-pad.l-pad.r)/Math.max(1,buckets.length);
    let svg='';
    buckets.forEach((b,i)=>{
      let y=h-pad.b;const x=pad.l+i*bw+bw*0.12,bw2=bw*0.76;
      subjKeys.forEach(k=>{
        const v=b.values[k]||0;if(!v)return;
        const bh=(h-pad.t-pad.b)*(v/max);y-=bh;
        svg+=`<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${bw2.toFixed(1)}" height="${bh.toFixed(1)}" fill="${subjectColor(k)}"/>`;
      });
      if(b.label)svg+=`<text x="${(x+bw2/2).toFixed(1)}" y="${h-6}" text-anchor="middle" class="stats-bar-label">${esc(b.label)}</text>`;
    });
    return `<svg viewBox="0 0 ${w} ${h}" class="stats-bar-svg">${svg}</svg>`;
  }
  function cumulativeAreaSVG(buckets,opts){
    opts=opts||{};const w=opts.w||620,h=opts.h||190,p={l:34,r:12,t:16,b:26};
    let running=0;const rows=buckets.map(b=>{running+=b.value;return {label:b.label,value:running}});
    const max=Math.max(1,...rows.map(r=>r.value));
    const pts=rows.map((r,i)=>({x:p.l+i*(w-p.l-p.r)/Math.max(1,rows.length-1),y:h-p.b-(r.value/max)*(h-p.t-p.b)}));
    const path=pts.map((q,i)=>(i?"L":"M")+q.x.toFixed(1)+" "+q.y.toFixed(1)).join(" ");
    const area=path+` L ${pts[pts.length-1].x} ${h-p.b} L ${pts[0].x} ${h-p.b} Z`;
    const step=Math.max(1,Math.ceil(rows.length/6));
  ...
  function timelineItem(e){
    const v=SUBJECTS[e.subject]||{icon:'📘',name:e.subject};
    const startMs=episodeStartMs(e),endMs=episodeEndMs(e);
    return `<div class="stats-timeline-item">
      <i class="stats-timeline-dot" style="background:${subjectColor(e.subject)}"></i>
      <div class="stats-timeline-body">
        <div class="stats-timeline-top"><b>${v.icon} ${esc(e.subjectLabel||v.name)}</b><span>${fmtClock(startMs)} - ${fmtClock(endMs)}</span></div>
        <div class="muted small">${esc(e.topic||'')}${e.topic?' • ':''}${fmtDuration(e.minutes)}${e.tests?` • ${fmt(e.tests)} تست`:''}</div>
      </div>
    </div>`;
  }

  /* ---- range renderers ---- */
  function renderStatsDay(body,nav){
    const d=statsState.anchor,dateStr=ymd(d),isToday=dateStr===today();
    nav.innerHTML=`<button class="btn" onclick="statsShift(-1)">◀</button><div class="stats-nav-label">${faDayLabel(d)}</div><button class="btn" onclick="statsShift(1)" ${isToday?'disabled':''}>▶</button>${isToday?'':'<button class="btn" onclick="statsToday()">امروز</button>'}`;
    const eps=state.episodes.filter(e=>e.date===dateStr).slice().sort((a,b)=>episodeStartMs(a)-episodeStartMs(b));
    const mins=totalMinutes(eps),tests=eps.reduce((a,e)=>a+(+e.tests||0),0),correct=eps.reduce((a,e)=>a+(+e.correct||0),0);
    const acc=tests?Math.round(correct/tests*100):null;
    const startMs=eps.length?Math.min(...eps.map(episodeStartMs)):null,endMs=eps.length?Math.max(...eps.map(episodeEndMs)):null;
    const breakdown=subjectBreakdown(eps);
    ...
    body.innerHTML=`
      <div class="statgrid stats-summary-grid">
        <div class="stat"><span class="label">کل زمان مطالعه</span><b>${fmtDuration(mins)}</b></div>
        <div class="stat"><span class="label">تعداد پارت</span><b>${fmt(eps.length)}</b></div>
        <div class="stat"><span class="label">تست‌های زده‌شده</span><b>${fmt(tests)}</b></div>
        <div class="stat"><span class="label">دقت</span><b>${acc!=null?acc+'٪':'—'}</b></div>
        <div class="stat"><span class="label">شروع مطالعه</span><b>${startMs?fmtClock(startMs):'—'}</b></div>
  ...
  function renderStatsWeek(body,nav){
    const weekStart=startOfWeek(statsState.anchor),weekEnd=addDays(weekStart,6);
    const isCurrentWeek=ymd(startOfWeek(new Date()))===ymd(weekStart);
    nav.innerHTML=`<button class="btn" onclick="statsShift(-1)">◀</button><div class="stats-nav-label">${faShortDate(weekStart)} تا ${faShortDate(weekEnd)}</div><button class="btn" onclick="statsShift(1)" ${isCurrentWeek?'disabled':''}>▶</button>${isCurrentWeek?'':'<button class="btn" onclick="statsToday()">این هفته</button>'}`;
    const dayLabels=["ش","ی","د","س","چ","پ","ج"];
    const days=[];
    for(let i=0;i<7;i++){const d=addDays(weekStart,i),key=ymd(d),eps=state.episodes.filter(e=>e.date===key);days.push({date:d,key,label:dayLabels[i],minutes:totalMinutes(eps),episodes:eps});}
    const weekEps=episodesBetween(weekStart,weekEnd),mins=totalMinutes(weekEps),avg=mins/7;
    const prevWeekStart=addDays(weekStart,-7),prevWeekEnd=addDays(weekStart,-1);
    const prevMins=totalMinutes(episodesBetween(prevWeekStart,prevWeekEnd));
    const delta=prevMins?Math.round((mins-prevMins)/prevMins*100):(mins>0?100:0);
    const studiedDays=days.filter(d=>d.minutes>0).length;
    const breakdown=subjectBreakdown(weekEps);
    const donutItems=Object.entries(breakdown).map(([k,v])=>({label:subjectDisplayNameSafe(k),value:v.minutes,color:subjectColor(k)})).filter(x=>x.value>0).sort((a,b)=>b.value-a.value);
    ...
    body.innerHTML=`
      ...
      <div class="statgrid stats-summary-grid">
        <div class="stat"><span class="label">مجموع هفته</span><b>${fmtDuration(mins)}</b></div>
        <div class="stat"><span class="label">میانگین روزانه</span><b>${fmtDuration(avg)}</b></div>
        <div class="stat"><span class="label">روزهای مطالعه‌شده</span><b>${studiedDays} از ۷</b></div>
  ...
  function renderStatsMonth(body,nav){
    const g=statsState.anchor,j=gregorianToJalali(g.getFullYear(),g.getMonth()+1,g.getDate());
    const daysInMonth=jalaliDaysInMonth(j.jy,j.jm);
    const monthStart=jalaliToGregorian(j.jy,j.jm,1),monthEnd=jalaliToGregorian(j.jy,j.jm,daysInMonth);
    const nd=new Date(),nowJ=gregorianToJalali(nd.getFullYear(),nd.getMonth()+1,nd.getDate());
    const isCurrentMonth=nowJ.jy===j.jy&&nowJ.jm===j.jm;
    nav.innerHTML=`<button class="btn" onclick="statsShift(-1)">◀</button><div class="stats-nav-label">${JALALI_MONTHS[j.jm-1]} ${fmt(j.jy)}</div><button class="btn" onclick="statsShift(1)" ${isCurrentMonth?'disabled':''}>▶</button>${isCurrentMonth?'':'<button class="btn" onclick="statsToday()">این ماه</button>'}`;
    const monthEps=episodesBetween(monthStart,monthEnd),mins=totalMinutes(monthEps);
    const activeDays=new Set(monthEps.map(e=>e.date)).size,avg=mins/daysInMonth;
    let prevJm=j.jm-1,prevJy=j.jy;if(prevJm<1){prevJm=12;prevJy--}
    const prevDaysInMonth=jalaliDaysInMonth(prevJy,prevJm);
    const prevStart=jalaliToGregorian(prevJy,prevJm,1),prevEnd=jalaliToGregorian(prevJy,prevJm,prevDaysInMonth);
    const prevMins=totalMinutes(episodesBetween(prevStart,prevEnd));
    const delta=prevMins?Math.round((mins-prevMins)/prevMins*100):(mins>0?100:0);
    const startIdx=(monthStart.getDay()+1)%7;
    const dayMinutesArr=[];let maxDayMin=1;
    for(let d=1;d<=daysInMonth;d++){const gd=jalaliToGregorian(j.jy,j.jm,d),key=ymd(gd),m=totalMinutes(state.episodes.filter(e=>e.date===key));dayMinutesArr.push({d,key,minutes:m});if(m>maxDayMin)maxDayMin=m;}
    const dayNames=["ش","ی","د","س","چ","پ","ج"];
    let calHtml=dayNames.map(x=>`<div class="stats-cal-name">${x}</div>`).join('');
    for(let i=0;i<startIdx;i++)calHtml+=`<div class="stats-cal-day empty"></div>`;
    dayMinutesArr.forEach(({d,key,minutes})=>{
      const lvl=minutes<=0?0:minutes<maxDayMin*0.25?1:minutes<maxDayMin*0.5?2:minutes<maxDayMin*0.75?3:4;
      const isToday=key===today();
      calHtml+=`<div class="stats-cal-day l${lvl} ${isToday?'today':''}" onclick="statsSelectDay('${key}')"><b>${fmt(d)}</b>${minutes>0?`<small>${fmtDuration(minutes)}</small>`:''}</div>`;
    });
    const breakdown=subjectBreakdown(monthEps);
    const donutItems=Object.entries(breakdown).map(([k,v])=>({label:subjectDisplayNameSafe(k),value:v.minutes,color:subjectColor(k)})).filter(x=>x.value>0).sort((a,b)=>b.value-a.value);
    const subjKeys=Object.keys(breakdown).sort((a,b)=>breakdown[b].minutes-breakdown[a].minutes).slice(0,8);
    const buckets=dayMinutesArr.map(({d,key})=>{
      const dayEps=state.episodes.filter(e=>e.date===key),values={};
      subjKeys.forEach(k=>values[k]=totalMinutes(dayEps.filter(e=>e.subject===k)));
    ...
    body.innerHTML=`
      <div class="statgrid stats-summary-grid">
        ...
        <div class="stat"><span class="label">میانگین روزانه</span><b>${fmtDuration(avg)}</b></div>
        <div class="stat"><span class="label">روزهای فعال</span><b>${activeDays} از ${daysInMonth}</b></div>
        <div class="stat"><span class="label">نسبت به ماه قبل</span><b style="color:${delta>=0?'var(--good)':'var(--bad)'}">${delta>=0?'▲':'▼'} ${Math.abs(delta)}٪</b></div>
      </div>
      <div class="card stats-card"><h3>تقویم مطالعه</h3><div class="stats-cal-grid">${calHtml}</div>
  ...
  function renderStatsTotal(body,nav){
    nav.innerHTML=`<div class="stats-nav-label">از اولین روز مطالعه تا امروز</div>`;
    const allEps=state.episodes,mins=totalMinutes(allEps),firstDate=firstEpisodeDate();
    const daysSinceStart=Math.max(1,Math.floor((new Date(today()+'T12:00:00')-new Date(firstDate+'T12:00:00'))/86400000)+1);
    const activeDaysSet=new Set(allEps.map(e=>e.date)),activeDays=activeDaysSet.size;
    const avgActive=activeDays?mins/activeDays:0;
    const totalTests=allEps.reduce((a,e)=>a+(+e.tests||0),0),totalCorrect=allEps.reduce((a,e)=>a+(+e.correct||0),0);
    const overallAcc=totalTests?Math.round(totalCorrect/totalTests*100):null;
    const sortedDates=Array.from(activeDaysSet).sort();
    let longest=0,cur=0,prevD=null;
    sortedDates.forEach(ds=>{const d=new Date(ds+'T12:00:00');if(prevD&&(d-prevD)===86400000)cur++;else cur=1;longest=Math.max(longest,cur);prevD=d;});
    const breakdown=subjectBreakdown(allEps);
    const donutItems=Object.entries(breakdown).map(([k,v])=>({label:subjectDisplayNameSafe(k),value:v.minutes,color:subjectColor(k)})).filter(x=>x.value>0).sort((a,b)=>b.value-a.value);
    const firstG=new Date(firstDate+'T12:00:00'),firstJ=gregorianToJalali(firstG.getFullYear(),firstG.getMonth()+1,firstG.getDate());
    const nd=new Date(),nowJ=gregorianToJalali(nd.getFullYear(),nd.getMonth()+1,nd.getDate());
    const monthBuckets=[];let jy=firstJ.jy,jm=firstJ.jm,guard=0;
    while((jy<nowJ.jy||(jy===nowJ.jy&&jm<=nowJ.jm))&&guard<240){
      const dim=jalaliDaysInMonth(jy,jm),mStart=jalaliToGregorian(jy,jm,1),mEnd=jalaliToGregorian(jy,jm,dim);
      const mMins=totalMinutes(episodesBetween(mStart,mEnd));
      monthBuckets.push({label:JALALI_MONTHS[jm-1].slice(0,3),value:mMins});
      jm++;if(jm>12){jm=1;jy++}
      guard++;
    }
    const rankingRows=Object.entries(breakdown).sort((a,b)=>b[1].minutes-a[1].minutes).map(([k,v])=>{
      const acc=v.tests?Math.round(v.correct/v.tests*100):null;
      return `<div class="stats-rank-row"><i style="background:${subjectColor(k)}"></i><b>${esc(subjectDisplayNameSafe(k))}</b><span>${fmtDuration(v.minutes)}</span><span>${fmt(v.episodes)} پارت</span><span>${acc!=null?acc+'٪ دقت':'—'}</span></div>`;
    }).join('');
    body.innerHTML=`
      <div class="statgrid stats-summary-grid">
        <div class="stat"><span class="label">مجموع کل زمان مطالعه</span><b>${fmtDuration(mins)}</b></div>
        <div class="stat"><span class="label">از شروع تا امروز</span><b>${fmt(daysSinceStart)} روز</b></div>
        <div class="stat"><span class="label">روزهای فعال</span><b>${fmt(activeDays)}</b></div>
        <div class="stat"><span class="label">میانگین روزهای فعال</span><b>${fmtDuration(avgActive)}</b></div>
        <div class="stat"><span class="label">بلندترین رشته پیوستگی</span><b>${fmt(longest)} روز</b></div>
  ...
  function renderStatistics(){
    const body=document.getElementById('statsBody'),nav=document.getElementById('statsNav');
    if(!body||!nav)return;
    if(statsState.range==='day')renderStatsDay(body,nav);
    else if(statsState.range==='week')renderStatsWeek(body,nav);
    else if(statsState.range==='month')renderStatsMonth(body,nav);
    else renderStatsTotal(body,nav);
  }
  window.renderStatistics=renderStatistics;
  window.setStatsRange=function(r){
    statsState.range=r;
    document.querySelectorAll('.stats-tab').forEach(b=>b.classList.toggle('active',b.dataset.range===r));
    renderStatistics();
  };
  window.statsShift=function(delta){
    if(statsState.range==='day')statsState.anchor=addDays(statsState.anchor,delta);
    else if(statsState.range==='week')statsState.anchor=addDays(statsState.anchor,delta*7);
    else if(statsState.range==='month'){
      const g=statsState.anchor,j=gregorianToJalali(g.getFullYear(),g.getMonth()+1,g.getDate());
      let jm=j.jm+delta,jy=j.jy;
      if(jm<1){jm=12;jy--}if(jm>12){jm=1;jy++}
      statsState.anchor=jalaliToGregorian(jy,jm,1);
    }
    renderStatistics();
  };
  window.statsToday=function(){const d=new Date();d.setHours(12,0,0,0);statsState.anchor=d;renderStatistics();};
  window.statsSelectDay=function(dateStr){
    statsState.range='day';statsState.anchor=new Date(dateStr+'T12:00:00');
    document.querySelectorAll('.stats-tab').forEach(b=>b.classList.toggle('active',b.dataset.range==='day'));
    renderStatistics();
  };

  const _origSwitchToPage2=window.switchToPage;
  window.switchToPage=function(name){
    _origSwitchToPage2(name);
